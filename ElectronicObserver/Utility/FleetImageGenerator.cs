...
namespace ElectronicObserver.Utility
{
	...
	public class FleetImageGenerator
	{
		...
		protected static readonly ShipParameterData[] ParameterDataList = new[] {
            ...
            new ShipParameterData( ResourceManager.IconContent.ParameterAircraft, "제공", ship => Calculator.GetAirSuperiority(ship).ToString() ),
        ...
        protected static readonly ShipParameterData[] ParameterDataList_JP = new[] {
        ...
        /// 詳細な情報を表示する、カード式の編成画像を出力します。
        /// </summary>
        public static Bitmap GenerateCardBitmap_JP(FleetImageArgument args)
        {
            ...
            using (var g = Graphics.FromImage(bitmap))
            {
                ...
                for (int fleetIndex = 0; fleetIndex < args.FleetIDs.Length; fleetIndex++)
                {
                    ...
                    for (int shipIndex = 0; shipIndex < maxShipCount; shipIndex++)
                    {
                        ...
                        // ship name
                        g.DrawString(ship.Name_JP, args.LargeFont, mainTextBrush, new Rectangle(shipPointer + GetAlignmentOffset(ContentAlignment.MiddleLeft, shipNameSize, shipNameAreaSize), shipNameSize), formatMiddleLeft);
                        shipPointer.X += shipNameSize.Width;
                        ...
                        // parameters
                        for (int parameterIndex = 0; parameterIndex < ParameterDataList_JP.Length; parameterIndex++)
                        {
                            Point parameterPointer = shipPointer + GetAlignmentOffset(ContentAlignment.TopCenter, parameterAreaSize, shipValuePaneSize) + new Size((parameterAreaUnitSize.Width + parameterAreaInnerMargin.Width) * (parameterIndex % 2), (parameterAreaUnitSize.Height + parameterAreaInnerMargin.Height) * (parameterIndex / 2));
                            var paramdata = ParameterDataList_JP[parameterIndex];
        ...
        /// カットイン式の編成画像を出力します。
        /// </summary>
        public static Bitmap GenerateCutinBitmap_JP(FleetImageArgument args)
        {
        ...
        /// コンパクトにまとめた、バナー式の編成画像を出力します。
        /// </summary>
        public static Bitmap GenerateBannerBitmap_JP(FleetImageArgument args)
        {
            int maxShipCount = Math.Max(args.FleetIDs.Select(id => KCDatabase.Instance.Fleet[id].Members.Count(i => i > 0)).DefaultIfEmpty(0).Max(), 6);
            ...
            using (var g = Graphics.FromImage(bitmap))
            {
                ...
                for (int fleetIndex = 0; fleetIndex < args.FleetIDs.Length; fleetIndex++)
                {
                    ...
                    for (int shipIndex = 0; shipIndex < maxShipCount; shipIndex++)
                    {
                        ...
                        {
                            // alternate drawing
                            g.DrawString(ship.Name_JP, args.MediumFont, mainTextBrush, new RectangleF(shipPointer.X, shipPointer.Y, ShipBannerSize.Width, ShipBannerSize.Height), formatTopLeft);
                        }
                        shipPointer.X += shipBannerSize.Width;
        ...
        public static Bitmap GenerateBaseAirCorpsImage_JP(FleetImageArgument args)
        {
            var formatMiddleLeft = GetStringFormat(ContentAlignment.MiddleLeft);
            var formatMiddleCenter = GetStringFormat(ContentAlignment.MiddleCenter);
            var formatMiddleRight = GetStringFormat(ContentAlignment.MiddleRight);
